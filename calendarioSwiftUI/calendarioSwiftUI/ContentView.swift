                    ...
                    HStack {
                        ForEach(1...7, id: \.self) { dia in
                            ...
                            ZStack{
                                ...
                                } else if calcDia(dia: dia, semana: semana) < inicio && calcDia(dia: dia, semana: semana) - inicio + 1 <= final{
                                    Circle().foregroundColor(.gray).opacity(0.3)
                                    Text("\(calcDia(dia: dia, semana: semana) - inicio + 1 + finalPassado)").opacity(0.3)
                                } else {
                                    Circle().foregroundColor(.gray).opacity(0.3)
                                    Text("\(calcDia(dia: dia, semana: semana) - inicio + 1 - final)").opacity(0.3)
                                }
                            }
                            
                        }
                    }
                    .frame(width: geometry.size.width - 25, height: 50, alignment: .center)
                }
